...
module.exports = (message, options) => {
  message = (message || 'Welcome to Whodunit, ladies and gentlemen!').trim();
  options = options || {};
   ...
   * explanation.
   *
   * When pisay is given a string, we create a duplicate with the ansi styling
  ...
  // Amount of characters of the whodunit character »column«      → `    /___A___\   /`
  const WHODUNIT_CHARACTER_WIDTH = 17;
  ...
  let TOTAL_CHARACTERS_PER_LINE = WHODUNIT_CHARACTER_WIDTH + DEFAULT_TOP_FRAME_WIDTH;

  // The speech bubble will overflow the whodunit character if the message is too long.
  ...
  if (options.maxLength) {
    ...
    if (maxLength < options.maxLength) {
      ...
      TOTAL_CHARACTERS_PER_LINE = maxLength + WHODUNIT_CHARACTER_WIDTH + topOffset;
    }
  }
  ...
  return wrap(strippedMessage, maxLength, {hard: true})
    ...
    .reduce((greeting, str, index, array) => {
      ...
      if (index === 0) {
        ...
        // The speech bubble will overflow the whodunit character if the message
