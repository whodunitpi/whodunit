...
var tmpdir = path.join(os.tmpdir(), 'whodunit-run-context');
...
describe('RunContext', function() {
  beforeEach(function() {
    ...
    this.Dummy = class extends Investigator {
      exec(...args) {
  ...
  describe('constructor', function() {
    it('accept path parameter', function(done) {
      var ctx = new RunContext(require.resolve('./fixtures/investigator-simple/app'));
    ...
    it('propagate investigator error events', function(done) {
      var onceDone = _.once(done);
      var error = new Error();
      var Dummy = helpers.createDummyInvestigator();
    ...
    it('accept investigator constructor parameter (and assign gen:test as namespace)', function(done) {
      this.ctx.on(
        'ready',
        function() {
    ...
    it('set namespace and resolved path in investigator', function(done) {
    ...
    it('run the investigator asynchronously', function(done) {
    ...
    it('only run an investigation once', function(done) {
      this.ctx.on(
        'end',
        function() {
          sinon.assert.calledOnce(this.execSpy);
  ...
  describe('#withArguments()', function() {
    it('provide arguments to the investigator when passed as Array', function(done) {
    ...
    it('provide arguments to the investigator when passed as String', function(done) {
  ...
  describe('#withOptions()', function() {
    it('provide options to the investigator', function(done) {
  ...
  describe('#withPrompts()', function() {
    it('is call automatically', function(done) {
      this.Dummy.prototype.askFor = function() {
        this.prompt(
          ...
          function(answers) {
            assert.equal(answers.whodunit, 'pass');
          }
        );
      };
    ...
    it('mock the prompt', function(done) {
      this.Dummy.prototype.askFor = function() {
        this.prompt(
          ...
          function(answers) {
            assert.equal(answers.whodunit, 'yes please');
          }
        );
      };

      this.ctx.withPrompts({ whodunit: 'yes please' }).on('end', done);
    });
    ...
    it('is chainable', function() {
      ...
      return this.ctx
        .withPrompts({ whodunit: 'yes please' })
        .withPrompts({ yo: 'yo man' })
        .toPromise();
    });
  });

  describe('#withInvestigators()', function() {
    it('register paths', function(done) {
      this.ctx.withInvestigators([require.resolve('./fixtures/investigator-simple/app')]).on(
        'ready',
        function() {
    ...
    it('register mocked investigator', function(done) {
      this.ctx.withInvestigators([[helpers.createDummyInvestigator(), 'dummy:gen']]).on(
        'ready',
        function() {
    ...
    it('allow multiple calls', function(done) {
      this.ctx
        .withInvestigators([require.resolve('./fixtures/investigator-simple/app')])
        .withInvestigators([[helpers.createDummyInvestigator(), 'dummy:gen']])
        .on(
          'ready',
          function() {
            assert(this.ctx.env.get('dummy:gen'));
  ...
  describe('#withLocalConfig()', function() {
    it('provides config to the investigator', function(done) {
      this.ctx
        .withLocalConfig({
          some: true,
          data: 'here'
        ...
        .on(
          'ready',
          function() {
            assert.equal(this.ctx.investigator.config.get('some'), true);
            assert.equal(this.ctx.investigator.config.get('data'), 'here');
            done();
          }.bind(this)
        );
    });
