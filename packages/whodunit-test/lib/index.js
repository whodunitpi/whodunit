...
/**
 * Answer prompt questions for the passed-in investigator
 * @param {Investigator} investigator - a whodunit investigator
 * @param {Object} answers - an object where keys are the
 *   investigators prompt names and values are the answers to
 *   the prompt questions
 * @example
 * mockPrompt(angular, {'bootstrap': 'Y', 'compassBoostrap': 'Y'});
 */

exports.mockPrompt = function(investigator, answers) {
  var promptModule = investigator.env.adapter.promptModule;
  answers = answers || {};
...
/**
 * Restore defaults prompts on an investigator.
 * @param {Investigator} investigator
 */
exports.restorePrompt = function(investigator) {
  investigator.env.adapter.promptModule.restoreDefaultPrompts();
};
 ...
 */
exports.mockLocalConfig = function(investigator, localConfig) {
  investigator.config.defaults(localConfig);
};

/**
 * Create a simple, dummy investigator
 */

exports.createDummyInvestigator = () =>
  class extends Investigator {
    test() {
      this.shouldRun = true;
    }
  };
...
/**
 * Create an investigator, using the given dependencies and controller arguments
 * Dependecies can be path (autodiscovery) or an array [<investigator>, <name>]
 *
 * @param {String} name - the name of the investigator
 * @param {Array} dependencies - paths to the investigators dependencies
 * @param {Array|String} args - arguments to the investigator;
 *   if String, will be split on spaces to create an Array
 * @param {Object} options - configuration for the investigator
 * @example
 *  var deps = ['../../app',
 *              '../../common',
 *              '../../controller',
 *              '../../main',
 *              [createDummyInvestigator(), 'testacular:app']
 *            ];
 * var angular = createInvestigator('angular:app', deps);
 */

exports.createInvestigator = function(name, dependencies, args, options) {
...
/**
 * Register a list of dependent investigators into the provided env.
 * Dependecies can be path (autodiscovery) or an array [<investigator>, <name>]
 *
 * @param {Array} dependencies - paths to the investigators dependencies
 */
...
/**
 * Run the provided Investigator
 * @param  {String|Function} InvestigatorOrNamespace - Investigator constructor or namespace
 * @return {RunContext}
 */

exports.run = function(InvestigatorOrNamespace, settings) {
  var RunContext = require('./run-context');
  return new RunContext(InvestigatorOrNamespace, settings);
};
