...
/**
 * The Completer is in charge of handling `pi-complete` behavior.
 * @constructor
...
class Completer {
   ...
   */
  complete(data, done) {
    if (data.last !== 'pi' && !data.last.startsWith('-')) {
      return this.investigator(data, done);
    }
    ...
    this.env.lookup(err => {
      ...
      const meta = this.env.getInvestigatorsMeta();
      const results = Object.keys(meta).map(this.item('pi'), this);
      done(null, results);
    });
  }

  /**
   * Investigator completion event done
   *
   * @param {String}   data   Environment object as parsed by tabtab
   * @param {Function} done   Callback to invoke with completion results
   */
  investigator(data, done) {
    const {last} = data;
