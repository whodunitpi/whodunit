...
describe('install route', () => {
  beforeEach(function () {
    ...
    this.env.registerStub(_.noop, 'investigator-unicorn');
  });
  ...
  describe('npm success with results', () => {
    beforeEach(function () {
      this.packages = [
        ...
        {
          name: 'investigator-unrelated',
          description: 'some description'
        },
        {
          name: 'investigator-unicorn-1',
        ...
        {
          name: 'investigator-blacklist-1',
          description: 'foo description'
        },
        {
          name: 'investigator-blacklist-2',
          description: 'foo description'
        },
        {
          name: 'investigator-blacklist-3',
          description: 'foo description'
        }
      ];

      this.blacklist = [
        'investigator-blacklist-1',
        'investigator-blacklist-2'
      ];
    ...
    it('filters already installed investigators and match search term', function (done) {
      let call = 0;
      this.sandbox.stub(inquirer, 'prompt').callsFake(arg => {
        ...
        if (call === 2) {
          const {choices} = arg[0];
          assert.equal(_.filter(choices, {value: 'investigator-foo'}).length, 1);
          assert.equal(_.filter(choices, {value: 'investigator-unicorn-1'}).length, 1);
          assert.equal(_.filter(choices, {value: 'investigator-unicorn'}).length, 0);
          assert.equal(_.filter(choices, {value: 'investigator-unrelated'}).length, 0);
          done();
        }
    ...
    it('filters blacklisted investigators and match search term', function (done) {
      let call = 0;
      this.sandbox.stub(inquirer, 'prompt').callsFake(arg => {
        call++;
        if (call === 1) {
        ...
        if (call === 2) {
          const {choices} = arg[0];
          assert.equal(_.filter(choices, {value: 'investigator-blacklist-1'}).length, 0);
          assert.equal(_.filter(choices, {value: 'investigator-blacklist-2'}).length, 0);
          assert.equal(_.filter(choices, {value: 'investigator-blacklist-3'}).length, 1);
          done();
        }
    ...
    it('install an investigator', function () {
      let call = 0;
      this.sandbox.stub(inquirer, 'prompt').callsFake(() => {
        call++;
        if (call === 1) {
        ...
        if (call === 2) {
          return Promise.resolve({toInstall: 'investigator-unicorn'});
        }
      ...
      return this.router.navigate('install').then(() => {
        sinon.assert.calledWith(this.spawn, 'npm', ['install', '--global', 'investigator-unicorn'], {stdio: 'inherit'});
