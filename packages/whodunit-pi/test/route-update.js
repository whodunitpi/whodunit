...
describe('update route', () => {
  ...
  it('allows updating investigators and return user to home screen', function () {
    const investigators = ['investigator-cat', 'investigator-unicorn'];
    ...
    return this.router.navigate('update').then(() => {
      sinon.assert.calledWith(
        this.crossSpawn,
        'npm',
        ['install', '--global'].concat(investigators)
      );
      sinon.assert.calledWith(this.insight.track, 'pipi', 'update');
      sinon.assert.calledWith(this.insight.track, 'pipi', 'updated');
