...
const helpers = require('@whodunit/test');
const { TestAdapter } = require('@whodunit/test/lib/adapter');
const Base = require('..');
const Conflicter = require('../lib/util/conflicter');

const tmpdir = path.join(os.tmpdir(), 'whodunit-base');
const resolveddir = path.join(os.tmpdir(), 'whodunit-base-investigator');

describe('Base', () => {
  beforeEach(helpers.setUpTestDirectory(tmpdir));

  beforeEach(function() {
    this.env = whodunit.createEnv([], { 'skip-install': true }, new TestAdapter());
  ...
  describe('constructor', () => {
    it('set the CWD where `.pi-rc.json` is found', function() {
    ...
    it('use the environment options', function() {
      ...
      const investigator = this.env.create('ember:model', {
        options: {
          'test-framework': 'jasmine'
        }
      });

      assert.equal(investigator.options['test-framework'], 'jasmine');
    });

    it('set investigator.options from constructor options', function() {
      const investigator = new Base({
        env: this.env,
        resolved: 'test',
        'test-framework': 'mocha'
      });

      assert.equal(investigator.options['test-framework'], 'mocha');
    });

    it('set options based on nopt arguments', function() {
      const investigator = new Base(['--foo', 'bar'], {
        env: this.env,
        resolved: 'test'
      });

      investigator.option('foo');
      assert.equal(investigator.options.foo, true);
      assert.deepEqual(investigator.args, ['bar']);
    });
    ...
    it('set arguments based on nopt arguments', function() {
      ...
      investigator.option('foo');
      investigator.argument('baz');
      assert.equal(investigator.options.baz, 'bar');
    });

    it('set options with false values', done => {
      const investigator = helpers
        .run(path.join(__dirname, './fixtures/options-investigator'))
        .withOptions({ testOption: false })
        .on('end', () => {
          assert.equal(investigator.options.testOption, false);
          done();
        });
    });
  ...
  describe('#determineAppname()', () => {
    ...
    it('returns appname from the current directory', function() {
      assert.equal(this.dummy.determineAppname(), 'whodunit base');
    });
  });

  describe('#run()', () => {
    beforeEach(function() {
      this.TestInvestigator = class extends Base {};
      _.extend(this.TestInvestigator.prototype, {
        exec: sinon.spy(),
        exec2: sinon.spy(),
        exec3: sinon.spy(),
        _private: sinon.spy(),
      ...
      this.execSpy = this.TestInvestigator.prototype.exec;

      this.testGen = new this.TestInvestigator([], {
        resolved: 'investigator-ember/all/index.js',
        namespace: 'dummy',
        env: this.env,
        'skip-install': true
      });
    ...
    it('run all methods in the given investigator', function() {
    ...
    it("don't try running prototype attributes", function() {
      this.TestInvestigator.prototype.prop = 'something';
      return this.testGen.run();
    });
    ...
    it('can emit error from async methods', function(done) {
      this.TestInvestigator.prototype.throwing = function() {
    ...
    it('can emit error from sync methods', function(done) {
      ...
      this.TestInvestigator.prototype.throwing = () => {
        throw error;
      };
    ...
    it('stop queue processing once an error is thrown', function() {
      ...
      this.TestInvestigator.prototype.afterError = spy;
    ...
    it('handle function returning promises as asynchronous', function() {
      ...
      this.TestInvestigator.prototype.first = () => {
      ...
      this.TestInvestigator.prototype.second = () => {
        spy2();
      };
    ...
    it('handle failing promises as errors', function(done) {
      this.TestInvestigator.prototype.failing = () => {
    ...
    it('run methods in series', function(done) {
      ...
      this.TestInvestigator.prototype.async1 = function() {
        async1Running = true;
        const cb = this.async();
      ...
      this.TestInvestigator.prototype.async2 = () => {
    ...
    it('throws if no method is available', function() {
      ...
      return gen.run().catch(err => {
        assert(err.toString().includes('This Investigator is empty'));
      });
    });

    it('will run non-enumerable methods', function() {
      class Investigator extends Base {}

      Object.defineProperty(Investigator.prototype, 'nonenumerable', {
        value: sinon.spy(),
        configurable: true,
        writable: true
      });

      const gen = new Investigator([], {
        resolved: 'dummy',
        namespace: 'dummy',
        env: this.env
      });
    ...
    it('ignore underscore prefixed method', function() {
      return this.testGen.run().then(() => {
        assert(this.TestInvestigator.prototype._private.notCalled);
      });
    });

    it('run methods in a queue hash', function() {
      return this.testGen.run().then(() => {
        assert(this.TestInvestigator.prototype.prompting.m1.calledOnce);
        assert(this.TestInvestigator.prototype.prompting.m2.calledOnce);
      });
    });

    it('ignore underscore prefixed method in a queue hash', function() {
      return this.testGen.run().then(() => {
        assert(this.TestInvestigator.prototype.prompting._private.notCalled);
      });
    });

    it('run named queued methods in order', function() {
      const initSpy = this.TestInvestigator.prototype.initializing;
      const promptSpy = this.TestInvestigator.prototype.prompting.m1;
    ...
    it('run queued methods in order even if not in order in prototype', function() {
      const initSpy = this.TestInvestigator.prototype.initializing;
      const execSpy = this.TestInvestigator.prototype.exec;
    ...
    it('commit mem-fs to disk', function() {
      ...
      this.TestInvestigator.prototype.writing = function() {
        this.fs.write(
    ...
    it('allow skipping file writes to disk', function() {
      ...
      const env = whodunit.createEnv([], { 'skip-install': true }, new TestAdapter(action));
      const testGen = new this.TestInvestigator([], {
        resolved: 'investigator/app/index.js',
        namespace: 'dummy',
        env
      });
    ...
    it('does not prompt again for skipped files', function() {
      ...
      // The composed investigator need to write at least one file for it to go through it's
      // file writing cycle
    ...
    it('does not pass config file to conflicter', function() {
      this.TestInvestigator.prototype.writing = function() {
        fs.writeFileSync(this.destinationPath('.pi-rc.json'), '{"foo": 3}');
        fs.writeFileSync(path.join(os.homedir(), '.pi-rc-global.json'), '{"foo": 3}');
    ...
    it('allows file writes in any priorities', function() {
      this.TestInvestigator.prototype.end = function() {
  ...
  describe('#argument()', () => {
    it('add a new argument to the investigator instance', function() {
    ...
    it('allows specifying default argument values', function() {
      const Investigator = class extends Base {
  ...
  describe('#option()', () => {
    it('add a new option to the set of investigator expected options', function() {
      // Every investigator have the --help options
      const investigator = new this.Dummy([], {
        env: this.env,
        resolved: 'test'
      });

      investigator.option('foo');
      assert.deepEqual(investigator._options.foo, {
  ...
  describe('#parseOptions()', () => {
    ...
    it('set investigator options', function() {
      this.dummy.parseOptions();
      assert.equal(this.dummy.options.foo, 'bar');
    });

    it('set investigator alias options', function() {
      this.dummy.parseOptions();
  ...
  describe('#composeWith()', () => {
    ...
    it('runs the composed investigators', function(done) {
      ...
      // investigator before the base one is ran.
      setTimeout(() => {
        this.dummy.run().then(() => {
    ...
    it('runs the composed Investigator class in the passed path', function() {
      this.stubPath = path.join(__dirname, 'fixtures/investigator-mocha');

      this.dummy.composeWith({
        Investigator: this.GenCompose,
        path: this.stubPath
      });
    ...
    describe('object as first argument', () => {
      it('fails for missing Investigator property', function() {
        ...
        assert.throws(
          ...
          error => error.message.indexOf('Missing Investigator property') >= 0
        );
      });
    ...
    it('run the composed investigator even if main investigator is already running.', function() {
      this.Dummy.prototype.writing = function() {
    ...
    it('pass options and arguments to the composed investigators', function() {
      this.dummy.composeWith('composed:gen', {
        foo: 'bar',
        'skip-install': true
      });
    ...
    describe('when passing a local path to an investigator', () => {
      beforeEach(function() {
        this.spy = sinon.spy();
        this.stubPath = path.join(__dirname, 'fixtures/investigator-mocha');
      ...
      it('runs the composed investigator', function() {
      ...
      it('sets correct metadata on the Investigator constructor', function() {
        this.dummy.composeWith(this.stubPath, {});
        return this.dummy.run().then(() => {
          assert.equal(this.spy.firstCall.thisValue.options.namespace, 'mocha');
          assert.equal(
  ...
  describe('#desc()', () => {
    it('update the internal description', function() {
      this.dummy.desc('A new desc for this investigator');
      assert.equal(this.dummy.description, 'A new desc for this investigator');
    });
  });
  ...
  describe('#help()', () => {
    it('return the expected help output', function() {
      ...
      const expected = [
        ...
        '',
        'A new desc for this investigator',
        '',
        'Options:',
  ...
  describe('#registerTransformStream()', () => {
    beforeEach(function() {
      this.filepath = path.join(os.tmpdir(), '/whodunit-transform-stream/filea.txt');
      this.TestInvestigator = class extends Base {};
      this.TestInvestigator.prototype.exec = sinon.spy();
      this.testGen = new this.TestInvestigator([], {
        resolved: 'investigator-ember/all/index.js',
        namespace: 'dummy',
        env: this.env
  ...
  describe('Events', () => {
    beforeEach(function() {
      ...
      Investigator.namespace = 'angular:app';
      Investigator.prototype.createSomething = () => {};
      Investigator.prototype.createSomethingElse = () => {};
    });

    it('emits the series of event on a specific investigator', function(done) {
      const angular = new this.Investigator([], {
        env: whodunit.createEnv([], {}, new TestAdapter()),
        resolved: __filename,
        'skip-install': true
      });
      ...
      angular
        // Run event, emitted right before running the investigator.
        .on('run', assertEvent('run'))
        // End event, emitted after the generation process, when every investigator
    ...
    it('only call the end event once (bug #402)', done => {
      class InvestigatorOnce extends Base {
        constructor(args, opts) {
          super(args, opts);
          this.sourceRoot(path.join(__dirname, 'fixtures'));
          this.destinationRoot(path.join(os.tmpdir(), 'whodunit-base-once'));
        }
      ...
      const investigatorOnce = new InvestigatorOnce([], {
        env: whodunit.createEnv([], {}, new TestAdapter()),
        resolved: __filename,
        'skip-install': true
      });

      investigatorOnce.on('end', () => {
      ...
      investigatorOnce.run();
    });

    it('triggers end event after all investigators methods are ran (#709)', done => {
      const endSpy = sinon.spy();
      const InvestigatorEnd = class extends Base {
        constructor(args, opts) {
          super(args, opts);
          this.on('end', () => {
      ...
      const investigatorEnd = new InvestigatorEnd([], {
        env: whodunit.createEnv([], {}, new TestAdapter()),
        resolved: __filename,
        'skip-install': true
      });
  ...
  describe('#rootInvestigatorName', () => {
    afterEach(() => {
      rimraf.sync(path.join(resolveddir, 'package.json'));
    });

    it('returns the default name', function() {
      assert.equal(this.dummy.rootInvestigatorName(), '*');
    });

    it('returns investigator name', function() {
      fs.writeFileSync(
        path.join(resolveddir, 'package.json'),
        '{ "name": "investigator-name" }'
      );
      assert.equal(this.dummy.rootInvestigatorName(), 'investigator-name');
    });
  });

  describe('#rootInvestigatorVersion', () => {
    afterEach(() => {
      rimraf.sync(path.join(resolveddir, 'package.json'));
    });

    it('returns the default version', function() {
      assert.equal(this.dummy.rootInvestigatorVersion(), '0.0.0');
    });

    it('returns investigator version', function() {
      fs.writeFileSync(path.join(resolveddir, 'package.json'), '{ "version": "1.0.0" }');
      assert.equal(this.dummy.rootInvestigatorVersion(), '1.0.0');
    });
  });
});
