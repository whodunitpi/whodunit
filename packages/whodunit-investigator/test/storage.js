...
const env = require('@whodunit/environment');
const helpers = require('@whodunit/test');
const Storage = require('../lib/util/storage');

const tmpdir = path.join(os.tmpdir(), 'whodunit-storage');
...
describe('Storage', () => {
  ...
  describe('#set()', () => {
    it('set values', function() {
      this.store.set('name', 'whodunit!');
      assert.equal(this.store.get('name'), 'whodunit!');
    });
    ...
    describe('@return', () => {
      ...
      it('the saved value (with key)', function() {
        assert.equal(this.store.set('name', 'whodunit!'), 'whodunit!');
      });
